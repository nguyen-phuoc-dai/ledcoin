...
const char * const BITCOIN_CONF_FILENAME = "led.conf";
const char * const BITCOIN_PID_FILENAME = "ledd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "led" is a composite category enabling all Led-related debug output
            if(ptrCategory->count(string("led"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LedCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LedCore
    // Mac: ~/Library/Application Support/LedCore
    // Unix: ~/.ledcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LedCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LedCore";
#else
    // Unix
    return pathRet / ".ledcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty led.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override led.conf
