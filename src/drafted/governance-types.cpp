...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "LedNetwork") type = LedNetwork;
    if(strType == "LedNetworkVariable") type = LedNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == LedNetwork) s = "LedNetwork";
    if(type == LedNetworkVariable) s = "LedNetworkVariable";
